...
advblocker.contentFiltering = (function (advblocker) {
    ...
    var ContentFilter = function (requestId, requestType, charset) {
        this.filter = advblocker.webRequest.filterResponseData(requestId);
        this.requestType = requestType;

        this.content = '';
        this.contentDfd = new advblocker.utils.Promise();
        ...
        this.filter.ondata = (event) => {
            if (!this.charset) {
                try {
                     ...
                     */
                    if (this.requestType === advblocker.RequestTypes.DOCUMENT ||
                        this.requestType === advblocker.RequestTypes.SUBDOCUMENT) {
            ...
            contentFilter.getContent()
                .then(function (content) {
                    ...
                    try {
                        ...
                        advblocker.console.error('Error while applying content filter to {0}. Error: {1}', requestUrl, ex);
                    }
                    contentFilter.write(content);
                }, function (error) {
                    advblocker.console.error('An error has occurred in content filter for request {0} to {1} - {2}. Error: {3}', requestId, requestUrl, requestType, error);
                    callback(null);
                });
        };
    };
    ...
    var DocumentParser = function () {
        if (typeof DOMParser === 'undefined') {
            advblocker.console.info('DOMParser object is not defined');
            this.parse = function () {
                return null;
            };
            return;
    ...
    var replaceRuleAllowedRequestTypeMask = (function () {
        var mask = 0;
        var replaceRuleAllowedRequestTypes = [advblocker.RequestTypes.DOCUMENT, advblocker.RequestTypes.SUBDOCUMENT, advblocker.RequestTypes.SCRIPT, advblocker.RequestTypes.STYLESHEET, advblocker.RequestTypes.XMLHTTPREQUEST];
        for (var i = 0; i < replaceRuleAllowedRequestTypes.length; i++) {
            var requestType = replaceRuleAllowedRequestTypes[i];
            mask |= advblocker.rules.UrlFilterRule.contentTypes[requestType];
        }
        return mask;
    })();
    ...
    var shouldApplyReplaceRule = function (requestType, contentType) {
        ...
        var responseContentFilteringSupported = advblocker.prefs.features && advblocker.prefs.features.responseContentFilteringSupported;
        if (!responseContentFilteringSupported) {
            return false;
        }

        var requestTypeMask = advblocker.rules.UrlFilterRule.contentTypes[requestType];
        if ((requestTypeMask & replaceRuleAllowedRequestTypeMask) === requestTypeMask) {
            return true;
        }

        if (requestType === advblocker.RequestTypes.OTHER && contentType) {
    ...
    var shouldApplyContentRules = function (requestType) {
        ...
        return requestType === advblocker.RequestTypes.DOCUMENT ||
    ...
    function applyContentRules(tab, requestUrl, referrerUrl, requestType, requestId, doc, rules) {
            ...
            if (elements) {
                for (var j = 0; j < elements.length; j++) {
                    ...
                    if (element.parentNode && deleted.indexOf(element) < 0) {
                        ...
                        advblocker.requestContextStorage.bindContentRule(requestId, rule, advblocker.utils.strings.elementToString(element));
        ...
        // Add <!DOCTYPE html ... >
        // https://github.com/advblockerTeam/advblockerBrowserExtension/issues/959
    ...
    function applyReplaceRules(tab, requestUrl, requestId, content, replaceRules) {
        ...
        // https://github.com/advblockerTeam/CoreLibs/issues/45
        ...
        if (appliedRules.length > 0) {
            advblocker.requestContextStorage.update(requestId, { replaceRules: appliedRules });
        }
    ...
    var apply = function (tab, requestUrl, referrerUrl, requestType, requestId, statusCode, method, contentType) {
        if (statusCode !== 200) {
            advblocker.console.debug('Skipping request to {0} - {1} with status {2}', requestUrl, requestType, statusCode);
            return;
        }

        if (method !== 'GET' && method !== 'POST') {
            advblocker.console.debug('Skipping request to {0} - {1} with method {2}', requestUrl, requestType, method);
            return;
        }
        ...
        if (charset && SUPPORTED_CHARSETS.indexOf(charset) < 0) {
            ...
            advblocker.console.warn('Skipping request to {0} - {1} with Content-Type {2}', requestUrl, requestType, contentType);
            return;
        }
        ...
        if (shouldApplyContentRules(requestType)) {
            contentRules = advblocker.webRequestService.getContentRules(tab, requestUrl);
            if (contentRules && contentRules.length === 0) {
                contentRules = null;
            }
        }
        ...
        if (shouldApplyReplaceRule(requestType, contentType)) {
            replaceRules = advblocker.webRequestService.getReplaceRules(tab, requestUrl, referrerUrl, requestType);
        ...
        advblocker.requestContextStorage.onContentModificationStarted(requestId);
        ...
        responseContentHandler.handleResponse(requestId, requestUrl, requestType, charset, (content) => {
            try {
                ...
                advblocker.requestContextStorage.onContentModificationFinished(requestId);
            }
        });
    };
