...
var WhiteListFilter = function (options) {
    ...
    const advblockerMode = ace.require('ace/mode/advblocker').Mode;
    editor.session.setMode(new advblockerMode());
...
var SyncSettings = function (options) {
    ...
    function bindControls() {
        ...
        document.querySelector('#advblockerSelectProvider').addEventListener('click', onProviderSelected('advblocker_SYNC'));
    ...
    function renderAuthorizedBlock() {
        ...
        if (currentProvider.isOAuthSupported && currentProvider.name === 'advblocker_SYNC') {
    ...
    function renderSyncSettings() {
        ...
        if (currentProvider) {
            ...
            switch (currentProvider.name) {
                case 'advblocker_SYNC':
                    document.querySelector('#advblockerSelectProvider').classList.add(activeClass);
                    break;
                case 'DROPBOX':
...
var Settings = function () {
    ...
    checkboxes.push(new Checkbox('#showInfoAboutadvblockerFullVersion', userSettings.names.DISABLE_SHOW_advblocker_PROMO_INFO, {
        negate: true,
    }));
...
const waitStorageInit = (advblocker) => {
    const timeoutMs = 500;
    if (!advblocker.localStorage.isInitialized()) {
        setTimeout(() => {
            waitStorageInit(advblocker);
        }, timeoutMs);
        return;
    }
...
backgroundPagePromise
    .then((page) => {
        waitStorageInit(page.advblocker);
    })
    .catch((e) => {
