 ...
 */
advblocker.localStorageImpl = advblocker.localStorageImpl || (function () {
 ...
 */
advblocker.localStorage = (function (advblocker, impl) {
    ...
    const setItem = function (key, value) {
        try {
            ...
            advblocker.console.error(`Error while saving item ${key} to the localStorage: ${ex}`);
        }
    };
...
})(advblocker, advblocker.localStorageImpl);

/**
 * Rules storage interface. Implementation depends on browser
 */
advblocker.rulesStorageImpl = advblocker.rulesStorageImpl || (function () {
    function notImplemented() {
        throw new Error('Not implemented');
    }
 ...
 */
advblocker.rulesStorage = (function (advblocker, impl) {
    ...
    const read = function (filterId, callback) {
        ...
        impl.read(filePath, (e, rules) => {
            if (e) {
                advblocker.console.error(`Error while reading rules from file ${filePath} cause: ${e}`);
            }
            callback(rules);
        });
    };
    ...
    const write = function (filterId, filterRules, callback) {
        ...
        impl.write(filePath, filterRules, (e) => {
            if (e) {
                advblocker.console.error(`Error writing filters to file ${filePath}. Cause: ${e}`);
            }
            callback();
        });
    };
...
})(advblocker, advblocker.rulesStorageImpl);
