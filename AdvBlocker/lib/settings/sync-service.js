 ...
 */
(function (api, advblocker, global) {
    ...
    function onSyncStatusUpdated() {
        advblocker.listeners.notifyListeners(advblocker.listeners.SYNC_STATUS_UPDATED, {
    ...
    function findCompatibility(remoteManifest, localManifest) {
        ...
        for (sectionName in merge) {
            ...
            if (!isSectionSyncEnabled(sectionName)) {
                advblocker.console.info('Section {0} sync is disabled by user settings', sectionName);
                continue;
            }
    ...
    function processManifest(callback) {
        ...
        var onManifestLoaded = function (remoteManifest) {
            ...
            advblocker.console.info('Processing remote manifest..');

            if (remoteManifest === false) {
                // Unable to fetch remote manifest
                advblocker.console.warn('Error loading remote manifest');
                callback(false);
                return;
            }
            ...
            if (!remoteManifest) {
                ...
                advblocker.console.info('Remote manifest does not exist');
            ...
            if (!validateManifest(remoteManifest)) {
                advblocker.console.warn('Remote manifest is not valid');
                callback(false);
                return;
            }
            ...
            if (!compatibility.canRead) {
                advblocker.console.warn('Protocol versions are not compatible');
                callback(false);
                return;
            }
            ...
            processSections(compatibility.sectionsLocalToRemote, compatibility.sectionsRemoteToLocal, function (success, updatedSections) {
                ...
                advblocker.console.info('Sections updated local-to-remote: {0}, remote-to-local: {1}', updatedSections.localToRemote.length, updatedSections.remoteToLocal.length);
                ...
                if (updatedSections.remoteToLocal.length > 0) {
                    advblocker.console.debug('Saving local manifest..');
                ...
                if (compatibility.canWrite && updatedSections.localToRemote.length > 0) {
                    ...
                    advblocker.console.debug('Saving remote manifest..');
        ...
        advblocker.console.debug('Loading remote manifest..');
    ...
    function processSections(sectionsLocalToRemote, sectionsRemoteToLocal, callback) {
        ...
        advblocker.utils.Promise.all(dfds).then(function () {
            advblocker.console.info('Sections updated successfully');
            callback(true, updatedSections);
        }, function () {
            advblocker.console.error('Sections update failed');
    ...
    function updateLocalToRemoteSection(section, callback) {
        ...
        advblocker.console.info('Updating local to remote: {0}', section.name);
        ...
        var onSectionLoaded = function (localSection) {
            ...
            if (!localSection) {
                advblocker.console.error('Local {0} section loading failed', sectionName);
                dfd.reject();
                return;
            }

            advblocker.console.debug('Local {0} section loaded', sectionName);

            saveRemoteSection(sectionName, localSection, function (success) {
                if (success) {
                    advblocker.console.info('Remote {0} section updated', sectionName);
                    callback(section);
                    dfd.resolve();
                } else {
                    advblocker.console.error('Remote {0} section update failed', sectionName);
                    dfd.reject();
                }
            });
        };
    ...
    function updateRemoteToLocalSection(section, callback) {
        ...
        var sectionName = section.name;
        advblocker.console.info('Updating remote to local: {0}', sectionName);

        var dfd = advblocker.utils.Promise();
        ...
        var onSectionLoaded = function (remoteSection) {
            ...
            if (!remoteSection) {
                advblocker.console.error('Remote {0} section loading failed', sectionName);
                dfd.reject();
                return;
            }

            advblocker.console.debug('Remote {0} section loaded', sectionName);

            saveLocalSection(sectionName, remoteSection, function (success) {
                if (success) {
                    advblocker.console.info('Local {0} section applied', sectionName);
                    callback(section);
                    dfd.resolve();
                } else {
                    advblocker.console.error('Local {0} section apply failed', sectionName);
                    dfd.reject();
                }
            });
        };
    ...
    function getLastSyncTimes() {
        if (lastSyncTimes === null) {
            var times = advblocker.localStorage.getItem(SYNC_LAST_SYNC_TIME_PROP);
    ...
    function setLastSyncTime(dateTime, providerName) {
        ...
        advblocker.localStorage.setItem(SYNC_LAST_SYNC_TIME_PROP, JSON.stringify(times));
    }
    ...
    function checkInfiniteLooping() {
        ...
        if (lastSyncTimesQueue.length > INF_LOOPS_CHECK_SIZE) {
            ...
            if (now - first < INF_LOOPS_CHECK_TIME) {
                ...
                advblocker.console.warn('Sync is disabled under suspicion of infinite loop.');
                lastSyncTimesQueue = [];
                return true;
            }
        }
    ...
    var toggleSyncStatus = function (enabled) {
        ...
        advblocker.localStorage.setItem(SYNC_STATUS_ENABLED_PROP, syncEnabled);
    ...
    var setSyncOptions = function (options) {
        advblocker.localStorage.setItem(SYNC_GENERAL_DISABLED_PROP, !options.syncGeneral);
        advblocker.localStorage.setItem(SYNC_FILTERS_DISABLED_PROP, !options.syncFilters);
        advblocker.localStorage.setItem(SYNC_EXTENSION_SPECIFIC_DISABLED_PROP, !options.syncExtensionSpecific);
    ...
    var init = function () {
        ...
        syncEnabled = String(advblocker.localStorage.getItem(SYNC_STATUS_ENABLED_PROP)) === 'true';

        syncOptions.syncGeneral = String(advblocker.localStorage.getItem(SYNC_GENERAL_DISABLED_PROP)) !== 'true';
        syncOptions.syncFilters = String(advblocker.localStorage.getItem(SYNC_FILTERS_DISABLED_PROP)) !== 'true';
        syncOptions.syncExtensionSpecific = String(advblocker.localStorage.getItem(SYNC_EXTENSION_SPECIFIC_DISABLED_PROP)) !== 'true';

        var providerName = advblocker.localStorage.getItem(SYNC_CURRENT_PROVIDER_PROP);
        if (providerName) {
    ...
    var setSyncProvider = function (providerName) {
        ...
        advblocker.console.info('Sync provider has been set to {0}', providerName);
        advblocker.localStorage.setItem(SYNC_CURRENT_PROVIDER_PROP, providerName);
    ...
    var syncSettings = function (callback) {
        ...
        advblocker.console.info('Synchronizing settings..');

        if (syncInProgress) {
            advblocker.console.info('Sync is already in progress...');
            onFinished(false);
            return;
        }
        syncInProgress = true;
        ...
        if (!syncEnabled) {
            advblocker.console.warn('Sync is disabled');
            onFinished(false);
            return;
        }

        if (!syncProvider) {
            advblocker.console.warn('Sync provider is not defined');
            onFinished(false);
            return;
        }
        ...
        if (!api.oauthService.isAuthorized(providerName)) {
            advblocker.console.warn('Sync provider is not authorized');
            onFinished(false);
            return;
        }
        ...
        processManifest(function (success) {
            ...
            if (success) {
                ...
                advblocker.console.info('Synchronizing settings finished');
            } else {
                advblocker.console.warn('Error synchronizing settings');
            }
    ...
    var getSyncStatus = function () {
        ...
        for (var i = 0; i < providers.length; i++) {
            ...
            if (provider.name === 'advblocker_SYNC') {
    ...
    var getDeviceName = function () {
        var deviceName = advblocker.localStorage.getItem(SYNC_DEVICE_NAME_PROP);
        if (!deviceName) {
            deviceName = advblocker.utils.browser.getClientId() +
                ' (' + advblocker.prefs.browser + ' ' + window.navigator.platform + ')';
        }
    ...
    var changeDeviceName = function (deviceName) {
        advblocker.localStorage.setItem(SYNC_DEVICE_NAME_PROP, deviceName);
    };
...
})(advblocker.sync, advblocker, window);
