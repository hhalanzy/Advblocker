...
(function (api, advblocker) {
    ...
    var GoogleDriveClient = (function () {
        ...
        function checkInvalidToken(status) {
            if (status === 401 || status === 403) {
                advblocker.listeners.notifyListeners(advblocker.listeners.SYNC_BAD_OR_EXPIRED_TOKEN, GOOGLE_PROVIDER_NAME);
            }
        }
        ...
        googleDriveFolderState.pollingTimeoutId = setTimeout(function () {
            syncListChanges()
                ...
                .catch(function (error) {
                    advblocker.console.error('Google Drive sync error {0}', error);
    ...
    var load = function (name, callback) {
        getFileIdByName(name)
            ...
            .catch(function (error) {
                advblocker.console.error('Google Drive sync error {0} {1}', name, error);
                callback(false);
            });
    };
    ...
    var init = function () {
        advblocker.console.info('Initialize {0} sync provider', GOOGLE_PROVIDER_NAME);
        startPolling();
    };

    var shutdown = function () {
        advblocker.console.info('Shutdown {0} sync provider', GOOGLE_PROVIDER_NAME);
