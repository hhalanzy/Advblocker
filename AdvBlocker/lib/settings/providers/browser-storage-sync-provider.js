...
(function (api, advblocker) {
    ...
    if (typeof browser === 'undefined' ||
        ...
        advblocker.console.info('Browser storage sync provider isn\'t supported');
        return;
    }
    ...
    var cropData = function (key, data, warn) {
        ...
        if (oversize > 0) {
            if (data.filters && data.filters["user-filter"] && data.filters["user-filter"].rules) {
                ...
                if (warn) {
                    advblocker.console.warn('The data is over quota limit and will be cropped approximately by ' + oversize + ' bytes');
                }
    ...
    api.syncProviders.register('BROWSER_SYNC', {
        title: advblocker.i18n.getMessage("sync_provider_browser_storage"),
        load: load,
        save: save,
        init: init,
        shutdown: shutdown
