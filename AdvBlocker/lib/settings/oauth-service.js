...
(function (api, advblocker) { // jshint ignore:line
    ...
    var getAccessTokens = function () {
        if (accessTokens === null) {
            var tokens = advblocker.localStorage.getItem(TOKEN_STORAGE_PROP);
    ...
    function clearToken(providerName) {
        ...
        advblocker.localStorage.setItem(TOKEN_STORAGE_PROP, JSON.stringify(accessTokens));
    }
    ...
    var setToken = function (providerName, token, csrfState, expires) {
        ...
        if (csrfState !== getOrGenerateCSRFState()) {
            advblocker.console.warn("Security token doesn't match");
            return false;
        }
        ...
        advblocker.console.info('Set OAuth token {0} for sync provider {1}', token, providerName);
    ...
    var authorize = function (providerName) {
        ...
        if (!provider.isOAuthSupported) {
            advblocker.console.warn('Sync provider doesn\'t support oauth');
            return;
        }
        advblocker.tabs.create({
            active: true,
            type: 'popup',
            url: provider.getAuthUrl('https://testauth.advblocker.com/oauth.html?provider=' + providerName, getOrGenerateCSRFState())
        });
    };
    ...
    var clearAndRevokeToken = function (providerName) {
        ...
        advblocker.console.info('Remove OAuth token \'{0}\' for sync provider {1}', token, providerName);
    };
