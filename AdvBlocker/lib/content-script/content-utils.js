...
(function () {
    ...
    /**
     * If isadvblockerTab we append div, else we append iframe
     * @param target
     * @param html
     * @param isadvblockerTab
     * @returns {HTMLElement}
     */
    const appendAlertElement = (target, html, isadvblockerTab) => {
        if (isadvblockerTab) {
    ...
    const genAlertHtml = (title, text) => {
        ...
        if (text && text.length > 0) {
            descBlock = `<div class="advblocker-popup-alert__desc">
                            ${text}
                        </div>`;
        }
        ...
        if (title && title.length > 0) {
            titleBlock = `<div class="advblocker-popup-alert__title">
                            ${title}
                        </div>`;
        }

        return `<div class="advblocker-popup-alert">
                    ${titleBlock}
                    ${descBlock}
                </div>`;
    };
    ...
    function showAlertPopup(message) {
        const { text, title, isadvblockerTab } = message;
        ...
        function appendPopup(count) {
            ...
            if (document.body) {
                const alertElement = appendAlertElement(document.body, alertDivHtml, isadvblockerTab);
                alertElement.classList.add('advblocker-alert-iframe');
                setTimeout(function () {
    ...
    function showVersionUpdatedPopup(message) {
        const {
            ...
            isadvblockerTab,
            isTabadvblockerDetected,
        } = message;

        // Do not show update message in the integration mode
        if (isTabadvblockerDetected) {
            return;
        }

        const updateIframeHtml = `<head></head>
                            <body>
                            <div id="advblocker-new-version-popup" class="advblocker-update-popup advblocker-update-popup--active">
                                <div id="advblocker-new-version-popup-close" class="advblocker-update-popup__close close-iframe"></div>
                                <div class="advblocker-update-popup__logo"></div>
                                <div class="advblocker-update-popup__title">
                                    ${title}
                                </div>
                                <div class="advblocker-update-popup__desc">
                                    ${description}
                                </div>
                                <div class="advblocker-update-popup__links">
                                    <a href="${changelogHref}" class="advblocker-update-popup__link close-iframe" target="_blank">
                                        ${changelogText}
                                    </a>
                                    <a href="#" class="advblocker-update-popup__link advblocker-update-popup__link--disable close-iframe disable-notifications">
                                        ${disableNotificationText}
                                    </a>
                                </div>
                                <div class="advblocker-update-popup__offer">
                                    ${offer}
                                </div>
                                <a href="${offerButtonHref}" class="advblocker-update-popup__btn close-iframe" target="_blank">
                                    ${offerButtonText}
                                </a>
                            </div>
                            </body>`;
        ...
        function appendPopup(count) {
            ...
            if (document.body && !isadvblockerTab) {
                const iframe = appendIframe(document.body, updateIframeHtml);
                iframe.classList.add('advblocker-update-iframe');
