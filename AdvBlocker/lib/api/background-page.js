...
(function (advblocker, browser) {
    'use strict';

    advblocker.runtime = (function () {
        const onMessage = {
            addListener(callback) {
                // https://developer.chrome.com/extensions/runtime#event-onMessage
                advblocker.runtimeImpl.onMessage.addListener((message, sender, sendResponse) => {
                    const senderOverride = Object.create(null);
                    if (sender.tab) {
                        senderOverride.tab = advblocker.tabsImpl.fromChromeTab(sender.tab);
                    }
    ...
    function shouldSkipRequest(details) {
        return details.tabId === advblocker.BACKGROUND_TAB_ID
    ...
    function parseRequestTypeFromUrl(url) {
        ...
        let requestType = advblocker.utils.browser.parseContentTypeFromUrlPath(path);
        if (requestType === null) {
            // https://code.google.com/p/chromium/issues/detail?id=410382
            requestType = advblocker.RequestTypes.OBJECT;
        }
        return requestType;
    }
     ...
     * @property {Number} requestType - request type {@link advblocker.RequestTypes}
    ...
    function getRequestDetails(details) {
        ...
        switch (details.type) {
            case 'main_frame':
                frameId = 0;
                requestType = advblocker.RequestTypes.DOCUMENT;
                break;
            case 'sub_frame':
        ...
        if (requestType === 'IMAGESET') {
            requestType = advblocker.RequestTypes.IMAGE;
        }

        if (requestType === advblocker.RequestTypes.OTHER) {
            requestType = parseRequestTypeFromUrl(details.url);
        }

        /**
         * Use `OTHER` type as a fallback
         * https://github.com/advblockerTeam/advblockerBrowserExtension/issues/777
         */
        if (!(requestType in advblocker.RequestTypes)) {
            requestType = advblocker.RequestTypes.OTHER;
        }
        ...
        if (details.tabId === advblocker.BACKGROUND_TAB_ID) {
     ...
     */
    advblocker.getURL = browser.extension.getURL;

    advblocker.backgroundPage = {};
    advblocker.backgroundPage.getWindow = function () {
    ...
    advblocker.app = {
         ...
         */
        getUrlScheme() {
            const url = advblocker.getURL('test.html');
            const index = url.indexOf('://');
            return url.substring(0, index);
        },
        ...
        addListener(callback) {
            ...
            browser.webNavigation.onCommitted.addListener((details) => {
                ...
                details.requestType = details.frameId === 0
                    ...
                    : advblocker.RequestTypes.SUBDOCUMENT;
    ...
    advblocker.contextMenus = browser.contextMenus;
})(advblocker, browser);
