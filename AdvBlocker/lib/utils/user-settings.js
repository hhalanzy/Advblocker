 ...
 * @constructor
 */
advblocker.settings = (function (advblocker) {
    ...
    var settings = {
        ...
        DISABLE_SHOW_advblocker_PROMO_INFO: 'show-info-about-advblocker-disabled',
        DISABLE_SAFEBROWSING: 'safebrowsing-disabled',
        DISABLE_SEND_SAFEBROWSING_STATS: 'safebrowsing-stats-disabled',
        DISABLE_FILTERING: 'advblocker-disabled',
    ...
    var propertyUpdateChannel = advblocker.utils.channels.newChannel();
    ...
    var defaultProperties = {
        get defaults() {
            return advblocker.lazyGet(this, 'defaults', function () {
                ...
                defaults[settings.DISABLE_SHOW_advblocker_PROMO_INFO] = (!advblocker.utils.browser.isWindowsOs() && !advblocker.utils.browser.isMacOs()) || advblocker.utils.browser.isEdgeBrowser();
                ...
                defaults[settings.USE_OPTIMIZED_FILTERS] = advblocker.prefs.mobile;
                ...
                defaults[settings.BLOCK_CHROME_CLIENT_DATA] = !!advblocker.utils.browser.isChromeBrowser();
    ...
    var getProperty = function (propertyName) {
        ...
        if (advblocker.localStorage.hasItem(propertyName)) {
            try {
                propertyValue = JSON.parse(advblocker.localStorage.getItem(propertyName));
            } catch (ex) {
                advblocker.console.error('Error get property {0}, cause: {1}', propertyName, ex);
            }
    ...
    const setProperty = (propertyName, propertyValue, options) => {
        advblocker.localStorage.setItem(propertyName, JSON.stringify(propertyValue));
        properties[propertyName] = propertyValue;
        ...
        if (!(options && options.syncSuppress)) {
            advblocker.listeners.notifyListeners(advblocker.listeners.SETTING_UPDATED, { propertyName, propertyValue });
        }
    };
    ...
    var isShowInfoAboutadvblockerFullVersion = function () {
        return !getProperty(settings.DISABLE_SHOW_advblocker_PROMO_INFO);
    };

    var changeShowInfoAboutadvblockerFullVersion = function (show, options) {
        setProperty(settings.DISABLE_SHOW_advblocker_PROMO_INFO, !show, options);
    };
    ...
    const disableShowadvblockerPromoInfo = function () {
        setProperty(settings.DISABLE_SHOW_advblocker_PROMO_INFO, true);
    };

    const isDisableShowadvblockerPromoInfo = function () {
        return getProperty(settings.DISABLE_SHOW_advblocker_PROMO_INFO);
    };
    ...
    api.isShowInfoAboutadvblockerFullVersion = isShowInfoAboutadvblockerFullVersion;
    api.changeShowInfoAboutadvblockerFullVersion = changeShowInfoAboutadvblockerFullVersion;
    ...
    api.disableShowadvblockerPromoInfo = disableShowadvblockerPromoInfo;
    api.isDisableShowadvblockerPromoInfo = isDisableShowadvblockerPromoInfo;
