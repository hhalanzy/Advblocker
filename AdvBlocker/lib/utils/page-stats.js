 ...
 * Global stats
 */
advblocker.pageStats = (function (advblocker) {
    'use strict';
    ...
    const TOTAL_GROUP = {
        groupId: 'total',
        groupName: advblocker.i18n ? advblocker.i18n.getMessage('popup_statistics_total') : 'Total',
    };
    ...
    const pageStatsHolder = {
         ...
         */
        get stats() {
            return advblocker.lazyGet(pageStatsHolder, 'stats', () => {
                let stats;
                try {
                    const json = advblocker.localStorage.getItem(pageStatisticProperty);
                    if (json) {
                        stats = JSON.parse(json);
                    }
                } catch (ex) {
                    advblocker.console.error('Error retrieve page statistic from storage, cause {0}', ex);
                }
                return stats || Object.create(null);
            });
        },

        save: advblocker.utils.concurrent.throttle(function () {
            advblocker.localStorage.setItem(pageStatisticProperty, JSON.stringify(this.stats));
        }, advblocker.prefs.statsSaveInterval),

        clear: function () {
            advblocker.localStorage.removeItem(pageStatisticProperty);
            advblocker.lazyGetClear(pageStatsHolder, 'stats');
        },
    };
    ...
    var updateStatsData = function (now, type, blocked, current) {
        ...
        if (advblocker.utils.dates.isSameHour(now, currentDate) && result.hours.length > 0) {
            result.hours[result.hours.length - 1] = updateStatsDataItem(type, blocked, result.hours[result.hours.length - 1]);
        } else {
            let diffHours = advblocker.utils.dates.getDifferenceInHours(now, currentDate);
        ...
        if (advblocker.utils.dates.isSameDay(now, currentDate) && result.days.length > 0) {
            result.days[result.days.length - 1] = updateStatsDataItem(type, blocked, result.days[result.days.length - 1]);
        } else {
            let diffDays = advblocker.utils.dates.getDifferenceInDays(now, currentDate);
        ...
        if (advblocker.utils.dates.isSameMonth(now, currentDate) && result.months.length > 0) {
            result.months[result.months.length - 1] = updateStatsDataItem(type, blocked, result.months[result.months.length - 1]);
        } else {
            let diffMonths = advblocker.utils.dates.getDifferenceInMonths(now, currentDate);
            while (diffMonths >= 2) {
                result.months.push(createStatsDataItem(null, 0));
                diffMonths -= 1;
            }
