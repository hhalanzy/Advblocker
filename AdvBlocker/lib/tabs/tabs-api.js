...
(function (advblocker) {
    ...
    advblocker.windowsImpl = advblocker.windowsImpl || (function () {
            ...
            return {
                ...
                onCreated: emptyListener, // callback (advblockerWin, nativeWin)
                onRemoved: emptyListener, // callback (windowId, nativeWin)
                onUpdated: emptyListener, // callback (advblockerWin, nativeWin, type) (Defined only for Firefox)

                create: noOpFunc,
                getLastFocused: noOpFunc, // callback (windowId, nativeWin)
                forEachNative: noOpFunc // callback (nativeWin, advblockerWin)
            };
        });

    advblocker.windows = (function (windowsImpl) {

        var advblockerWin = { // jshint ignore:line
            windowId: 1,
            type: 'normal' // 'popup'
        };
        ...
        var advblockerWindows = Object.create(null); // windowId => advblockerWin

        windowsImpl.forEachNative(function (nativeWin, advblockerWin) {
            advblockerWindows[advblockerWin.windowId] = advblockerWin;
        });

        var onCreatedChannel = advblocker.utils.channels.newChannel();
        var onRemovedChannel = advblocker.utils.channels.newChannel();

        windowsImpl.onCreated.addListener(function (advblockerWin) {
            advblockerWindows[advblockerWin.windowId] = advblockerWin;
            onCreatedChannel.notify(advblockerWin);
        });

        windowsImpl.onRemoved.addListener(function (windowId) {
            var advblockerWin = advblockerWindows[windowId];
            if (advblockerWin) {
                onRemovedChannel.notify(advblockerWin);
                delete advblockerWindows[windowId];
            }
        });
        ...
        var getLastFocused = function (callback) {
            windowsImpl.getLastFocused(function (windowId) {
                var metadata = advblockerWindows[windowId];
                if (metadata) {
                    callback(metadata[0]);
                }
            });
        ...
        return {
            ...
            onCreated: onCreatedChannel,    // callback(advblockerWin)
            onRemoved: onRemovedChannel,    // callback(advblockerWin)

            create: create,
            getLastFocused: getLastFocused // callback (advblockerWin)
        };

    })(advblocker.windowsImpl);

    advblocker.tabsImpl = advblocker.tabsImpl || (function () {
    ...
    advblocker.tabs = (function (tabsImpl) {

        var advblockerTab = { // jshint ignore:line
            tabId: 1,
            url: 'url',
            title: 'Title',
            incognito: false,
        ...
        var advblockerTabFrame = { // jshint ignore:line
            frameId: 1,
            url: 'url',
        ...
        var onUpdatedChannel = advblocker.utils.channels.newChannel();

        // Fires when the active tab in a window changes.
        var onActivatedChannel = advblocker.utils.channels.newChannel();
        ...
        var getActive = function (callback) {
            tabsImpl.getActive(function (tabId) {
                ...
                if (tab) {
                    ...
                    // TODO[Edge]: Relates to Edge Bug https://github.com/advblockerTeam/advblockerBrowserExtension/issues/481
        ...
        var recordTabFrame = function (tabId, frameId, url, domainName) {
            ...
            if (!tab && frameId === 0) {
                ...
                // https://github.com/advblockerTeam/advblockerBrowserExtension/issues/481
                tab = {
                    tabId: tabId,
                    url: url,
                    status: 'loading',
