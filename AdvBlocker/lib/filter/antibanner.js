 ...
 */
advblocker.antiBannerService = (function (advblocker) {

    // Add synthetic user filter
    var userFilter = { filterId: advblocker.utils.filters.USER_FILTER_ID };
    ...
    var requestFilter = new advblocker.RequestFilter();
    ...
    let filtersUpdatePeriod = advblocker.settings.getFiltersUpdatePeriod();
    ...
    const UPDATE_REQUEST_FILTER_EVENTS = [
        advblocker.listeners.UPDATE_FILTER_RULES,
        advblocker.listeners.FILTER_ENABLE_DISABLE,
        advblocker.listeners.FILTER_GROUP_ENABLE_DISABLE,
    ];
    ...
    const SAVE_FILTER_RULES_TO_STORAGE_EVENTS = [
        advblocker.listeners.UPDATE_FILTER_RULES,
        advblocker.listeners.ADD_RULES,
        advblocker.listeners.REMOVE_RULE,
    ];
    ...
    function initialize(options, callback) {
        ...
        var notifyApplicationUpdated = function (runInfo) {
            setTimeout(function () {
                advblocker.listeners.notifyListeners(advblocker.listeners.APPLICATION_UPDATED, runInfo);
        ...
        var onSubscriptionLoaded = function (runInfo) {
            ...
            if (runInfo.isFirstRun) {
                ...
                advblocker.applicationUpdateService.onUpdate(runInfo, initRequestFilter);
         ...
         */
        advblocker.applicationUpdateService.getRunInfo(async (runInfo) => {
            // Load subscription from the storage
            await advblocker.subscriptions.init();
    ...
    var stop = function () {
        ...
        requestFilter = new advblocker.RequestFilter();
        advblocker.listeners.notifyListeners(advblocker.listeners.REQUEST_FILTER_UPDATED, getRequestFilterInfo());
    };
    ...
    var addAntiBannerFilter = (filterId, callback) => {
        ...
        const onFilterLoaded = (success) => {
            if (success) {
                ...
                advblocker.listeners.notifyListeners(advblocker.listeners.FILTER_ADD_REMOVE, filter);
            }
            callback(success);
        };
    ...
    function selectFilterIdsToUpdate(forceUpdate, filtersToUpdate) {
        ...
        const filters = filtersToUpdate || advblocker.subscriptions.getFilters();
        ...
        for (let i = 0; i < filters.length; i += 1) {
            ...
            const group = advblocker.subscriptions.getGroup(filter.groupId);
    ...
    const checkAntiBannerFiltersUpdate = (forceUpdate, successCallback, errorCallback, filters) => {
        ...
        advblocker.console.info('Start checking filters updates');
        ...
        advblocker.console.info('Checking updates for {0} filters', totalToUpdate);
            ...
            loadFiltersFromBackend(filterMetadataList, (success, filterIds) => {
                if (success) {
                    const filters = filterIds
                        .map(advblocker.subscriptions.getFilter)
                        .filter(f => f);
        ...
        const onLoadFilterMetadataList = (success, filterMetadataList) => {
            if (success) {
                ...
                for (let i = 0; i < filterMetadataList.length; i += 1) {
                    ...
                    const filter = advblocker.subscriptions.getFilter(filterMetadata.filterId);
                    if (filter && filterMetadata.version && advblocker.utils.browser.isGreaterVersion(filterMetadata.version, filter.version)) {
                        advblocker.console.info(`Updating filter ${filter.filterId} to version ${filterMetadata.version}`);
    ...
    function updateCustomFilters(customFilterIds, callback) {
        ...
        const promises = customFilterIds.map(filterId => new Promise((resolve) => {
            const filter = advblocker.subscriptions.getFilter(filterId);
            ...
            advblocker.subscriptions.updateCustomFilter(filter.customUrl, {}, onUpdate);
        }));
        ...
        Promise.all(promises).then((filters) => {
            ...
            if (updatedFilters.length > 0) {
                ...
                advblocker.console.info(`Updated custom filters with ids: ${filterIdsString}`);
            }
    ...
    function resetFiltersVersion() {
        ...
        var filters = advblocker.subscriptions.getFilters();
     ...
     * Loads state info from the storage and then updates advblocker.subscription.groups properly
     * @private
     */
    function loadGroupsStateInfo() {
        // Load filters state from the storage
        const groupsStateInfo = advblocker.filtersState.getGroupsState();
     ...
     * Loads this data from the storage and then updates advblocker.subscription.filters property
     *
     * @private
     */
    function loadFiltersVersionAndStateInfo() {
        ...
        var filtersVersionInfo = advblocker.filtersState.getFiltersVersion();
        // Load filters state from the storage
        var filtersStateInfo = advblocker.filtersState.getFiltersState();
    ...
    function onFiltersLoadedFromStorage(rulesFilterMap, callback) {
        ...
        var async = advblocker.requestFilter.isReady();
        var asyncStep = 1000;
        advblocker.console.info('Starting request filter initialization. Async={0}', async);

        // Empty request filter
        var newRequestFilter = new advblocker.RequestFilter();
        ...
        if (requestFilterInitTime === 0) {
            ...
            advblocker.listeners.notifyListeners(advblocker.listeners.APPLICATION_INITIALIZED);
        }
        ...
        function isEmptyRulesFilterMap(rulesFilterMap) {
            ...
            var userFilterId = advblocker.utils.filters.USER_FILTER_ID;
        ...
        var requestFilterInitialized = function () {
            ...
            advblocker.listeners.notifyListeners(advblocker.listeners.REQUEST_FILTER_UPDATED, getRequestFilterInfo());
            advblocker.console.info('Finished request filter initialization in {0} ms. Rules count: {1}', (new Date().getTime() - start), newRequestFilter.rulesCount);
            ...
            if (newRequestFilter.rulesCount === 0 && !reloadedRules) {
                // https://github.com/advblockerTeam/advblockerBrowserExtension/issues/205
                advblocker.console.info('No rules have been found - checking filter updates');
                reloadAntiBannerFilters();
                reloadedRules = true;
            } else if (newRequestFilter.rulesCount > 0 && reloadedRules) {
                advblocker.console.info('Filters reloaded, deleting reloadRules flag');
                reloadedRules = false;
            }
        };
        ...
        const addRules = function (filterId, rulesTexts, startIdx, endIdx) {
            ...
            const isTrustedFilter = advblocker.subscriptions.isTrustedFilter(filterId);
            ...
            for (let i = startIdx; i < rulesTexts.length && i < endIdx; i += 1) {
                ...
                const rule = advblocker.rules.builder.createRule(ruleText, filterId, isTrustedFilter);
        ...
        var fillRequestFilterAsync = function () {
            ...
            for (let filterId in rulesFilterMap) { // jshint ignore:line
                // To number
                filterId = filterId - 0;
                if (filterId !== advblocker.utils.filters.USER_FILTER_ID) {
            ...
            // https://github.com/advblockerTeam/advblockerBrowserExtension/issues/117
            const userFilterId = advblocker.utils.filters.USER_FILTER_ID;
        ...
        var fillRequestFilterSync = function () {
            ...
            // User filter should be the last
            // https://github.com/advblockerTeam/advblockerBrowserExtension/issues/117
            var userRules = rulesFilterMap[advblocker.utils.filters.USER_FILTER_ID];
            addRules(advblocker.utils.filters.USER_FILTER_ID, userRules, 0, userRules.length);
            requestFilterInitialized();
        };
    ...
    function createRequestFilter(callback) {
        ...
        var start = new Date().getTime();
        advblocker.console.info('Starting loading filter rules from the storage');
        ...
        var loadAllFilterRulesDone = function () {
            advblocker.console.info('Finished loading filter rules from the storage in {0} ms', (new Date().getTime() - start));
        ...
        const loadFilterRulesFromStorage = (filterId, rulesFilterMap) => new Promise(resolve => {
            advblocker.rulesStorage.read(filterId, (rulesText) => {
                if (rulesText) {
        ...
        const loadFilterRules = function () {
            ...
            promises.push(loadFilterRulesFromStorage(advblocker.utils.filters.USER_FILTER_ID, rulesFilterMap));
    ...
    function addFiltersChangeEventListener() {
        ...
        const processEventsHistory = function () {
            ...
            if (needCreateRequestFilter) {
                ...
                advblocker.utils.Promise.all(dfds).then(createRequestFilter);
            } else {
        ...
        advblocker.listeners.addListener(function (event, filter, rules) {
            switch (event) {
        ...
        advblocker.listeners.addListener(function (event, group) {
            switch (event) {
    ...
    function processSaveFilterRulesToStorageEvents(filterId, events) {
        ...
        advblocker.rulesStorage.read(filterId, function (loadedRulesText) {
            ...
            for (var i = 0; i < events.length; i++) {
                ...
                switch (eventType) {
                    case advblocker.listeners.ADD_RULES:
                        ...
                        advblocker.console.debug("Add {0} rules to filter {1}", eventRules.length, filterId);
                        break;
                    case advblocker.listeners.REMOVE_RULE:
                        var actionRule = eventRules[0];
                        advblocker.utils.collections.removeAll(loadedRulesText, actionRule);
                        advblocker.console.debug("Remove {0} rule from filter {1}", actionRule, filterId);
                        break;
                    case advblocker.listeners.UPDATE_FILTER_RULES:
                        loadedRulesText = eventRules;
                        advblocker.console.debug("Update filter {0} rules count to {1}", filterId, eventRules.length);
                        break;
                }
            }

            advblocker.console.debug('Save {0} rules to filter {1}', loadedRulesText.length, filterId);
            advblocker.rulesStorage.write(filterId, loadedRulesText, function () {
                dfd.resolve();
                if (filterId === advblocker.utils.filters.USER_FILTER_ID) {
                    advblocker.listeners.notifyListeners(advblocker.listeners.UPDATE_USER_FILTER_RULES, getRequestFilterInfo());
                }
            });
        });
    ...
    function subscribeToFiltersChangeEvents() {
        ...
        var onUsedOptimizedFiltersChange = advblocker.utils.concurrent.debounce(reloadAntiBannerFilters, RELOAD_FILTERS_DEBOUNCE_PERIOD);

        advblocker.settings.onUpdated.addListener(function (setting) {
            if (setting === advblocker.settings.USE_OPTIMIZED_FILTERS) {
                onUsedOptimizedFiltersChange();
                return;
            }
            if (setting === advblocker.settings.DISABLE_COLLECT_HITS) {
                getRequestFilter().cssFilter.dirty = true;
            }
            if (setting === advblocker.settings.FILTERS_UPDATE_PERIOD) {
    ...
    function scheduleUpdate() {
        ...
        scheduleUpdateTimeoutId = setTimeout(() => {
            try {
                ...
                advblocker.console.error('Error update filters, cause {0}', ex);
            }
            scheduleUpdate();
    ...
    function scheduleFiltersUpdate(isFirstRun) {
        filtersUpdatePeriod = advblocker.settings.getFiltersUpdatePeriod();
        // First run delay
        if (isFirstRun) {
     ...
     *
     * @param filterId Filter identifier
     * @returns {*} Filter got from advblocker.subscriptions.getFilter
     * @private
     */
    ...
    function loadFilterRules(filterMetadata, forceRemote, callback) {
        ...
        advblocker.listeners.notifyListeners(advblocker.listeners.START_DOWNLOAD_FILTER, filter);

        const successCallback = (filterRules) => {
            advblocker.console.info('Retrieved response from server for filter {0}, rules count: {1}',
                filter.filterId,
                filterRules.length);
            ...
            advblocker.listeners.notifyListeners(advblocker.listeners.UPDATE_FILTER_RULES, filter, filterRules);
            callback(true);
        };

        const errorCallback = (cause) => {
            advblocker.console.error('Error retrieved response from server for filter {0}, cause: {1}', filter.filterId, cause || '');
            delete filter._isDownloading;
            advblocker.listeners.notifyListeners(advblocker.listeners.ERROR_DOWNLOAD_FILTER, filter);
            callback(false);
        };

        advblocker.backend.loadFilterRules(filter.filterId,
            forceRemote,
            advblocker.settings.isUseOptimizedFiltersEnabled()).then(successCallback, errorCallback);
    }
    ...
    function loadFiltersMetadataFromBackend(filterIds, callback) {
        ...
        const loadSuccess = (filterMetadataList) => {
            advblocker.console.debug('Retrieved response from server for {0} filters, result: {1} metadata', filterIds.length, filterMetadataList.length);
            callback(true, filterMetadataList);
        };

        const loadError = (request, cause) => {
            advblocker.console.error('Error retrieved response from server for filters {0}, cause: {1} {2}', filterIds, request.statusText, cause || '');
            callback(false);
        };

        advblocker.backend.loadFiltersMetadata(filterIds, loadSuccess, loadError);
    }
    ...
    var addUserFilterRules = function (rulesText) {
        ...
        for (var i = 0; i < rulesText.length; i++) {
            var rule = advblocker.rules.builder.createRule(rulesText[i], advblocker.utils.filters.USER_FILTER_ID);
            if (rule !== null) {
                rules.push(rule);
            }
        }
        ...
        advblocker.listeners.notifyListeners(advblocker.listeners.ADD_RULES, userFilter, rulesText);
        advblocker.listeners.notifyListeners(advblocker.listeners.UPDATE_USER_FILTER_RULES, getRequestFilterInfo());
    ...
    var updateUserFilterRules = function (rulesText) {
        advblocker.listeners.notifyListeners(advblocker.listeners.UPDATE_FILTER_RULES, userFilter, rulesText);
        advblocker.listeners.notifyListeners(advblocker.listeners.UPDATE_USER_FILTER_RULES, getRequestFilterInfo());
    };
    ...
    var removeUserFilterRule = function (ruleText) {
        var rule = advblocker.rules.builder.createRule(ruleText, advblocker.utils.filters.USER_FILTER_ID);
        if (rule !== null) {
            requestFilter.removeRule(rule);
        }
        advblocker.listeners.notifyListeners(advblocker.listeners.REMOVE_RULE, userFilter, [ruleText]);
    };
 ...
 */
advblocker.requestFilter = (function (advblocker) {

    'use strict';

    var antiBannerService = advblocker.antiBannerService;
 ...
 */
advblocker.filtersState = (function (advblocker) {
    ...
    var getFiltersVersion = function () {
        ...
        try {
            var json = advblocker.localStorage.getItem(FILTERS_VERSION_PROP);
            if (json) {
                filters = JSON.parse(json);
            }
        } catch (ex) {
            advblocker.console.error('Error retrieve filters version info, cause {0}', ex);
        }
        return filters;
    };
    ...
    var getFiltersState = function () {
        var filters = Object.create(null);
        try {
            var json = advblocker.localStorage.getItem(FILTERS_STATE_PROP);
            if (json) {
                filters = JSON.parse(json);
            }
        } catch (ex) {
            advblocker.console.error('Error retrieve filters state info, cause {0}', ex);
        }
        return filters;
    };
    ...
    const getGroupsState = function () {
        ...
        try {
            const json = advblocker.localStorage.getItem(GROUPS_STATE_PROP);
            if (json) {
                groups = JSON.parse(json);
            }
        } catch (e) {
            advblocker.console.error('Error retrieve groups state info, cause {0}', e);
        }
        return groups;
    };
    ...
    const updateFilterVersion = function (filter) {
        ...
        advblocker.localStorage.setItem(FILTERS_VERSION_PROP, JSON.stringify(filters));
    };
    ...
    const updateFilterState = function (filter) {
        ...
        advblocker.localStorage.setItem(FILTERS_STATE_PROP, JSON.stringify(filters));
    };
    ...
    const updateGroupState = function (group) {
        ...
        advblocker.localStorage.setItem(GROUPS_STATE_PROP, JSON.stringify(groups));
    };

    // Add event listener to persist filter metadata to local storage
    advblocker.listeners.addListener(function (event, payload) {
        switch (event) {
            case advblocker.listeners.SUCCESS_DOWNLOAD_FILTER:
 ...
 */
advblocker.filters = (function (advblocker) {
    ...
    const offerFilters = (callback) => {
        ...
        const filterIds = [
            advblocker.utils.filters.ENGLISH_FILTER_ID,
            advblocker.utils.filters.SEARCH_AND_SELF_PROMO_FILTER_ID,
        ];
        if (advblocker.prefs.mobile) {
            filterIds.push(advblocker.utils.filters.MOBILE_ADS_FILTER_ID);
        }
        filterIds.concat(advblocker.subscriptions.getLangSuitableFilters());
        callback(filterIds);
    };
     ...
     */
    const getEnabledFilters = () => advblocker.subscriptions.getFilters()
        .filter(f => f.installed && f.enabled);

    const getEnabledFiltersFromEnabledGroups = () => {
        const filters = advblocker.subscriptions.getFilters();
        const enabledGroupsIds = advblocker.subscriptions.getGroups()
            .filter(g => g.enabled)
            .map(g => g.groupId);
    ...
    var isFilterEnabled = function (filterId) {
        var filter = advblocker.subscriptions.getFilter(filterId);
    ...
    const enableGroup = function (groupId, options) {
        ...
        group.enabled = true;
        advblocker.listeners.notifyListeners(advblocker.listeners.FILTER_GROUP_ENABLE_DISABLE, group);
    ...
    const enableFilter = (filterId, options) => {
        ...
        if (!advblocker.subscriptions.groupHasEnabledStatus(groupId) || forceGroupEnable) {
            enableGroup(groupId, options);
        }
        advblocker.listeners.notifyListeners(advblocker.listeners.FILTER_ENABLE_DISABLE, filter);
        advblocker.listeners.notifyListeners(advblocker.listeners.SYNC_REQUIRED, options);
        return true;
    };
    ...
    const addAndEnableFilters = (filterIds, callback, options) => {
        ...
        filterIds = advblocker.utils.collections.removeDuplicates(filterIds.slice(0));
    ...
    const disableFilters = function (filterIds, options) {
        filterIds = advblocker.utils.collections.removeDuplicates(filterIds.slice(0)); // Copy array to prevent parameter mutation
    ...
    var uninstallFilters = function (filterIds, options) {
        ...
        for (var i = 0; i < filterIds.length; i++) {
            ...
            advblocker.console.debug("Uninstall filter {0}", filter.filterId);
    ...
    var removeFilter = function (filterId, options) {
        ...
        if (!filter.customUrl) {
            advblocker.console.error('Filter {0} is not custom and could not be removed', filter.filterId);
            return;
        }

        advblocker.console.debug('Remove filter {0}', filter.filterId);
    ...
    const loadCustomFilter = function (url, options, successCallback, errorCallback) {
        advblocker.console.info('Downloading custom filter from {0}', url);
        ...
        advblocker.subscriptions.updateCustomFilter(url, options, function (filterId) {
            if (filterId) {
                advblocker.console.info('Custom filter downloaded');
    ...
    const loadCustomFilterInfo = (url, options, successCallback, errorCallback) => {
        advblocker.console.info(`Downloading custom filter info from ${url}`);
        if (!url) {
            errorCallback();
            return;
        }

        advblocker.subscriptions.getCustomFilterInfo(url, options, (result = {}) => {
            const { error, filter } = result;
            if (filter) {
                advblocker.console.info('Custom filter data downloaded');
                successCallback(filter);
                return;
            }
            errorCallback(error);
