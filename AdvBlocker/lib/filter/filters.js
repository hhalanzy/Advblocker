...
(function (advblocker) {
    ...
    const RequestFilter = function () {
        ...
        // https://kb.advblocker.com/en/general/how-to-create-your-own-ad-filters#badfilter-modifier
        this.badFilterRules = {};

        // Filter that applies URL blocking rules
        // Basic rules: https://kb.advblocker.com/en/general/how-to-create-your-own-ad-filters#basic-rules
        this.urlBlockingFilter = new advblocker.rules.UrlFilter([], this.badFilterRules);

        // Filter that applies whitelist rules
        // Exception rules: https://kb.advblocker.com/en/general/how-to-create-your-own-ad-filters#exceptions-modifiers
        this.urlWhiteFilter = new advblocker.rules.UrlFilter([], this.badFilterRules);

        // Filter that applies CSS rules
        // https://kb.advblocker.com/en/general/how-to-create-your-own-ad-filters#cosmetic-rules
        this.cssFilter = new advblocker.rules.CssFilter();

        // Filter that applies JS rules
        // https://kb.advblocker.com/en/general/how-to-create-your-own-ad-filters#javascript-rules
        this.scriptFilter = new advblocker.rules.ScriptFilter();

        // Filter that applies CSP rules
        // https://kb.advblocker.com/en/general/how-to-create-your-own-ad-filters#csp-modifier
        this.cspFilter = new advblocker.rules.CspFilter();

        // Filter that applies cookie rules
        // https://github.com/advblockerTeam/advblockerBrowserExtension/issues/961
        this.cookieFilter = new advblocker.rules.CookieFilter();

        // Filter that applies stealth rules
        // https://kb.advblocker.com/en/general/how-to-create-your-own-ad-filters#stealth-modifier
        this.stealthFilter = new advblocker.rules.UrlFilter([], this.badFilterRules);

        // Filter that applies replace rules
        // https://kb.advblocker.com/en/general/how-to-create-your-own-ad-filters#replace-modifier
        this.replaceFilter = new advblocker.rules.ReplaceFilter([], this.badFilterRules);

        // Filter that applies HTML filtering rules
        // https://kb.advblocker.com/en/general/how-to-create-your-own-ad-filters#html-filtering-rules
        this.contentFilter = new advblocker.rules.ContentFilter();
    ...
    RequestFilter.prototype = {
        ...
        addRule(rule) {
            if (rule === null || !rule.ruleText) {
                advblocker.console.error('FilterRule must not be null');
                return;
            }

            if (rule instanceof advblocker.rules.UrlFilterRule) {
                if (typeof rule.isIgnored === 'function' && rule.isIgnored()) {
                    advblocker.console.debug(`FilterRule with $extension modifier was omitted. Rule text: "${rule.ruleText}"`);
                    return;
                }
                if (rule.isCspRule()) {
            ...
            } else if (rule instanceof advblocker.rules.CssFilterRule) {
                this.cssFilter.addRule(rule);
            } else if (rule instanceof advblocker.rules.ScriptFilterRule) {
                this.scriptFilter.addRule(rule);
            } else if (rule instanceof advblocker.rules.ScriptletRule) {
                this.scriptFilter.addRule(rule);
            } else if (rule instanceof advblocker.rules.CompositeRule) {
                rule.rules.forEach(this.addRule.bind(this));
            } else if (rule instanceof advblocker.rules.ContentFilterRule) {
         ...
         * http://advblocker.com/en/filterrules.html#hideRules
         *
        ...
        getSelectorsForUrl(url, options) {
            const domain = advblocker.utils.url.getHost(url);

            const { CSS_INJECTION_ONLY } = advblocker.rules.CssFilter;
            const cssInjectionOnly = (options & CSS_INJECTION_ONLY) === CSS_INJECTION_ONLY;

            if (!cssInjectionOnly
                && advblocker.webRequestService.isCollectingCosmeticRulesHits()) {
                /**
         ...
         * http://advblocker.com/en/filterrules.html#javascriptInjection
         *
         * @param url Page URL
        ...
        getScriptsForUrl(url, debug) {
            ...
            const config = {
                debug,
                engine: 'extension',
                version: advblocker.app && advblocker.app.getVersion && advblocker.app.getVersion(),
            };
        ...
        getScriptsStringForUrl(url, tab) {
            const debug = advblocker.filteringLog && advblocker.filteringLog.isOpen();
            const scriptRules = this.getScriptsForUrl(url, debug);

            const isFirefox = advblocker.utils.browser.isFirefoxBrowser();
            const isOpera = advblocker.utils.browser.isOperaBrowser();
            ...
            if (debug) {
                const domainName = advblocker.utils.url.getDomainName(url);
                scriptRules.forEach((scriptRule) => {
                    if (scriptRule.rule.isDomainSpecific(domainName)) {
                        advblocker.filteringLog.addScriptInjectionEvent(
                            tab,
                            url,
        ...
        findWhiteListRule(requestUrl, referrer, requestType) {
            const refHost = advblocker.utils.url.getHost(referrer);
            const thirdParty = advblocker.utils.url.isThirdPartyRequest(requestUrl, referrer);
        ...
        findRuleForRequest(requestUrl, documentUrl, requestType, documentWhitelistRule) {
            const documentHost = advblocker.utils.url.getHost(documentUrl);
            const thirdParty = advblocker.utils.url.isThirdPartyRequest(requestUrl, documentUrl);
        ...
        _findRuleForRequest(requestUrl, documentHost, requestType, thirdParty, documentWhiteListRule) {
            advblocker.console.debug('Filtering http request for url: {0}, document: {1}, requestType: {2}', requestUrl, documentHost, requestType);
    ...
    advblocker.RequestFilter = RequestFilter;
})(advblocker);
