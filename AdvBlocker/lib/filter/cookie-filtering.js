 ...
 */
advblocker.cookieFiltering = (function (advblocker) {
    ...
    const addCookieLogEvent = (tab, cookieName, cookieValue, cookieDomain, cookieThirdParty, rules, isModifyingCookieRule) => {
        for (let i = 0; i < rules.length; i += 1) {
            advblocker.filteringLog.addCookieEvent(tab, cookieName, cookieValue, cookieDomain, advblocker.RequestTypes.COOKIE, rules[i], isModifyingCookieRule, cookieThirdParty);
        }
    };
    ...
    const apiRemoveCookie = (name, url) => {
        return new Promise((resolve) => {
            browser.cookies.remove({ url, name }, () => {
                ...
                if (ex) {
                    advblocker.console.error('Error remove cookie {0} - {1}: {2}', name, url, ex);
                }
                resolve();
            });
        });
    ...
    const apiUpdateCookie = (apiCookie, url) => {
        ...
        return new Promise((resolve => {
            browser.cookies.set(update, () => {
                const ex = browser.runtime.lastError;
                if (ex) {
                    advblocker.console.error('Error update cookie {0} - {1}: {2}', apiCookie.name, url, ex);
                }
                resolve();
            });
        }));
    ...
    const processModifySetCookieByRules = (tab, setCookie, cookieDomain, thirdParty, header, rules) => {
        ...
        if (rules && rules.length > 0) {
            header.value = advblocker.utils.cookie.serialize(setCookie);
    ...
    var filterRequestHeaders = function (requestId, requestHeaders) {
        ...
        const cookieHeader = advblocker.utils.browser.findHeaderByName(requestHeaders, 'Cookie');
        const cookies = advblocker.utils.cookie.parseCookie(cookieHeader ? cookieHeader.value : null);
        if (!cookies) {
            return false;
        }
        ...
        const rules = advblocker.webRequestService.getCookieRules(tab, requestUrl, referrerUrl, requestType);
        const stealthRules = advblocker.stealthService.getCookieRules(requestUrl, referrerUrl, requestType);
        if ((!rules || rules.length === 0) &&
    ...
    var filterResponseHeaders = function (requestId, responseHeaders) {
        ...
        const referrerUrl = context.originUrl || context.referrerUrl;
        const requestType = context.requestType;
        const requestHost = advblocker.utils.url.getHost(requestUrl);
        ...
        while (iResponseHeaders--) {
            ...
            const setCookie = advblocker.utils.cookie.parseSetCookie(header.value);
            if (!setCookie) {
                continue;
            }
            ...
            // Marks requests without referrer as first-party.
            // It's important to prevent removing google auth cookies. (for requests in background tab)
            const thirdParty = referrerUrl && advblocker.utils.url.isThirdPartyRequest(cookieUrl, referrerUrl);
            const rules = advblocker.webRequestService.getCookieRules(tab, cookieUrl, referrerUrl, requestType);
            ...
            if (!ignoreStealthRules) {
                const stealthRules = advblocker.stealthService.getCookieRules(cookieUrl, referrerUrl, requestType);
    ...
    const modifyCookies = (requestId) => {
        ...
        if (advblocker.frames.shouldStopRequestProcess(tab)) {
            advblocker.console.debug('Tab is whitelisted or OFF');
            cookiesMap.delete(requestId);
            return false;
        }
