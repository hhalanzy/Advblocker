 ...
 *
 * This API is exposed via advblocker.requestContextStorage:
 *
...
(function (advblocker) {
    ...
    const record = (requestId, requestUrl, referrerUrl, originUrl, requestType, tab) => {
        ...
        if (requestType === advblocker.RequestTypes.DOCUMENT && !contexts.has(requestId)) {
            advblocker.filteringLog.clearEventsByTabId(tab.tabId);
        }
        ...
        advblocker.filteringLog.addHttpRequestEvent(tab, requestUrl, referrerUrl, requestType, null, eventId);
    };
    ...
    const recordEmulated = (requestUrl, referrerUrl, requestType, tab, requestRule) => {
        advblocker.filteringLog.addHttpRequestEvent(tab, requestUrl, referrerUrl, requestType, requestRule);
        advblocker.webRequestService.recordRuleHit(tab, requestRule, requestUrl);
    };
    ...
    const update = (requestId, update) => {
        ...
        if ('requestRule' in update) {
            ...
            advblocker.filteringLog.bindRuleToHttpRequestEvent(context.tab,
                context.requestRule,
                context.eventId);
        }
    ...
    const remove = (requestId) => {
        ...
        if (context.requestState === States.DONE) {
            ...
            if (requestRule) {
                advblocker.filteringLog.bindRuleToHttpRequestEvent(tab, requestRule, context.eventId);
                ruleHitsRecords.push(requestRule);
            }

            if (cspRules) {
                for (let cspRule of cspRules) {
                    advblocker.filteringLog.addHttpRequestEvent(tab, requestUrl, referrerUrl, advblocker.RequestTypes.CSP, cspRule);
                }
                ruleHitsRecords = ruleHitsRecords.concat(cspRules);
            }

            if (stealthActions) {
                advblocker.filteringLog.bindStealthActionsToHttpRequestEvent(tab, stealthActions, context.eventId);
            }
        }
        ...
        if (context.contentModifyingState === States.DONE) {
            ...
            if (replaceRules) {
                advblocker.filteringLog.bindReplaceRulesToHttpRequestEvent(tab, replaceRules, context.eventId);
            ...
            if (contentRules) {
                for (let contentRule of contentRules) {
                    ...
                    for (let element of elements) {
                        advblocker.filteringLog.addCosmeticEvent(tab, element, requestUrl, context.requestType, contentRule);
                    }
        ...
        for (let i = 0; i < ruleHitsRecords.length; i += 1) {
            advblocker.webRequestService.recordRuleHit(tab, ruleHitsRecords[i], requestUrl);
        }
    ...
    // Expose
    advblocker.requestContextStorage = {
        get,
        record,
        recordEmulated,
        update,
