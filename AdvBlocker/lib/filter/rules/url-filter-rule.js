...
(function (advblocker, api) {
    ...
    var isFirefoxBrowser = advblocker.utils.browser.isFirefoxBrowser();
    ...
    function getAsciiDomainRule(ruleText) {
        try {
            ...
            return advblocker.utils.strings.replaceAll(ruleText, domain, advblocker.utils.url.toPunyCode(domain));
        } catch (ex) {
            advblocker.console.error("Error getAsciiDomainRule from {0}, cause {1}", ruleText, ex);
            return "";
        }
    }
    ...
    function parseRuleDomain(ruleText, parseOptions) {
        try {
            ...
            for (i = 0; i < startsWith.length; i++) {
                ...
                if (advblocker.utils.strings.startWith(ruleText, start)) {
            ...
            advblocker.console.error("Error parsing domain from {0}, cause {1}", ruleText, ex);
            return null;
        }
    }
    ...
    function extractRegexpShortcut(ruleText) {
        ...
        if (reText.indexOf('?') !== -1) {
            ...
            // And with those using ? special character: https://github.com/advblockerTeam/advblockerBrowserExtension/issues/978
            return null;
        }
    ...
    function parseRuleText(ruleText) {
        ...
        if (advblocker.utils.strings.startWith(urlRuleText, api.FilterRule.MASK_WHITE_LIST)) {
        ...
        /**
         * https://github.com/advblockerTeam/advblockerBrowserExtension/issues/517
        ...
        if (advblocker.utils.strings.startWith(urlRuleText, api.UrlFilterRule.MASK_REGEX_RULE) &&
            advblocker.utils.strings.endsWith(urlRuleText, api.UrlFilterRule.MASK_REGEX_RULE) &&
            !advblocker.utils.strings.contains(urlRuleText, api.UrlFilterRule.REPLACE_OPTION + '=')) {
    ...
    function validateCspRule(rule) {
        ...
        if (rule.cspDirective) {
            ...
            /**
             * https://github.com/advblockerTeam/advblockerBrowserExtension/issues/685#issue-228287090
     ...
     * https://github.com/advblockerTeam/advblockerForWindows/issues/591
     */
    function ReplaceOption(option) {
        ...
        const parts = advblocker.utils.strings.splitByDelimiterWithEscapeCharacter(option, '/', ESCAPE_CHARACTER, true);
    ...
    function RedirectOption(option) {
        const getRedirectUrl = () => advblocker.rules.RedirectFilterService.buildRedirectUrl(option);
    ...
    function CookieOption(option) {
        ...
        if (advblocker.utils.strings.startWith(cookieName, '/') && advblocker.utils.strings.endsWith(cookieName, '/')) {
     ...
     * http://advblocker.com/en/filterrules.html#baseRules
     */
    var UrlFilterRule = function (rule, filterId) {
        ...
        this.isRegexRule = advblocker.utils.strings.startWith(urlRuleText, UrlFilterRule.MASK_REGEX_RULE) &&
            advblocker.utils.strings.endsWith(urlRuleText, UrlFilterRule.MASK_REGEX_RULE) ||
            urlRuleText === '' ||
    ...
    UrlFilterRule.prototype.getUrlRegExp = function () {
        ...
        if (!this.urlRegExp) {
            ...
            try {
                ...
                //malformed regexp
                advblocker.console.error('Error create regexp from {0}', urlRegExpSource);
     ...
     * @param requestType           Request type (one of advblocker.RequestTypes)
    ...
    /**
     * Does not inject advblocker javascript to page
     *
     * @return If true - we do not inject advblocker js to page matching this rule
     */
    ...
    /**
     * If empty is true than advblocker will return empty response
    ...
    UrlFilterRule.prototype._loadOptions = function (options) {
        ...
        let optionsParts = advblocker.utils.strings.splitByDelimiterWithEscapeCharacter(options, api.FilterRule.COMA_DELIMITER, ESCAPE_CHARACTER, false);
        ...
        for (let i = 0; i < optionsParts.length; i++) {
            ...
            switch (optionName) {
                ...
                case UrlFilterRule.REDIRECT_OPTION: {
                    ...
                    const redirectSource = advblocker.rules.RedirectFilterService.hasRedirect(optionValue);
                    if (!redirectSource) {
                ...
                case UrlFilterRule.REPLACE_OPTION:
                    ...
                    const responseContentFilteringSupported = advblocker.prefs.features
                        && advblocker.prefs.features.responseContentFilteringSupported;
    ...
    if (advblocker.prefs.platform === 'chromium'
        || advblocker.prefs.platform === 'webkit') {
    ...
    UrlFilterRule.options = {
        ...
        /**
         * If this option is enabled, advblocker won't apply generic CSS rules to the web page.
         */
        GENERICHIDE: 1 << 2,

        /**
         * If this option is enabled, advblocker won't apply generic UrlFilter rules to the web page.
         */
        GENERICBLOCK: 1 << 3,
         ...
         * the proper operation of advblocker Assistant
         */
        JSINJECT: 1 << 4,
         ...
         * advblocker will try to&nbsp;automatically close the browser tab.
         */
        BLOCK_POPUPS: 1 << 7,

        /**
         * For any address matching blocking rule with this option
         * advblocker will return internal redirect response (307)
         */
        EMPTY_RESPONSE: 1 << 8,
