...
(function (advblocker, api) {
    ...
    const _createRule = function (ruleText, filterId, isTrustedFilter) {
        ...
        try {
            ...
            if (api.FilterRule.findRuleMarker(ruleText, api.ContentFilterRule.RULE_MARKERS, api.ContentFilterRule.RULE_MARKER_FIRST_CHAR)) {
                let responseContentFilteringSupported = advblocker.prefs.features && advblocker.prefs.features.responseContentFilteringSupported;
            ...
            if (api.FilterRule.findRuleMarker(ruleText, api.ScriptFilterRule.RULE_MARKERS, api.ScriptFilterRule.RULE_MARKER_FIRST_CHAR)) {
                if (api.ScriptletRule.isadvblockerScriptletRule(ruleText)) {
            ...
            advblocker.console.debug('Cannot create rule from filter {0}: {1}, cause {2}', filterId || 0, ruleText, ex);
        }
    ...
    /**
     * Convert rules to advblocker syntax and create rule
     *
     * @param {string} ruleText Rule text
     * @param {number} filterId Filter identifier
    ...
    const createRule = (ruleText, filterId, isTrustedFilter = true) => {
        ...
        try {
            ...
            advblocker.console.debug('Cannot convert rule from filter {0}: {1}, cause {2}', filterId || 0, ruleText, ex);
        }
        if (!conversionResult) {
            return null;
        }
