...
(function (advblocker, api) {
    ...
    FilterRule.prototype = {
        ...
        /**
         * Loads $domain option.
         * http://advblocker.com/en/filterrules.html#hideRulesDomainRestrictions
         * http://advblocker.com/en/filterrules.html#advanced
         *
         * @param domains List of domains. Examples: "example.com|test.com" or "example.com,test.com"
         */
        loadDomains(domains) {
            if (advblocker.utils.strings.isEmpty(domains)) {
                return;
            }
            ...
            for (let i = 0; i < parts.length; i += 1) {
                ...
                if (domain.trim().length === 0) {
                    // https://github.com/advblockerTeam/advblockerBrowserExtension/issues/1242
                    throw `Error load $domain options from "${domains}", because after split one of them is empty`;
                }
                if (advblocker.utils.strings.startWith(domain, '~')) {
                    domainName = advblocker.utils.url.toPunyCode(domain.substring(1).trim());
                    if (!advblocker.utils.strings.isEmpty(domainName)) {
                    ...
                    domainName = advblocker.utils.url.toPunyCode(domain.trim());
                    if (!advblocker.utils.strings.isEmpty(domainName)) {
        ...
        isRestricted(domainName) {
            ...
            if (restrictedDomains) {
                return advblocker.utils.url.isDomainOrSubDomainOfAny(
                    domainName,
                    restrictedDomains
                );
            }
        ...
        addRestrictedDomains(domains) {
            if (domains) {
                ...
                restrictedDomains = advblocker.utils.collections.removeDuplicates((restrictedDomains || []).concat(domains));
                this.setRestrictedDomains(restrictedDomains);
            }
        },
        ...
        removeRestrictedDomains(domains) {
            if (domains) {
                ...
                for (let i = 0; i < domains.length; i++) {
                    advblocker.utils.collections.remove(restrictedDomains, domains[i]);
                }
                this.setRestrictedDomains(restrictedDomains);
            }
        },
     ...
     * https://github.com/advblockerTeam/advblockerBrowserExtension/issues/34
     * and
     * https://github.com/advblockerTeam/advblockerBrowserExtension/issues/1079
     */
