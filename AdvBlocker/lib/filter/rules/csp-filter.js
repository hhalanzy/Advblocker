...
(function (advblocker, api) {
    ...
    api.CspFilter = function (rules) {
        ...
        function findCspRules(url, documentHost, thirdParty, requestType) {
             ...
             * We can't pass `advblocker.RequestTypes.DOCUMENT` to findRules `function`, because $DOCUMENT modifier has other meaning for the rules.
             * So if we pass `advblocker.RequestTypes.OTHER` we won't match rules with $SUBDOCUMENT modifier, as we expected
             *
             * For example:
             * rule1 = '||$csp'
             * rule2 = '||$csp,subdocument'
             * rule3 = '||$csp,~subdocument'
             * findCspRules(advblocker.RequestTypes.SUBDOCUMENT) = [rule1, rule2];
             * findCspRules(advblocker.RequestTypes.DOCUMENT) = [rule1, rule3];
             */
            requestType = requestType === advblocker.RequestTypes.SUBDOCUMENT ? requestType : advblocker.RequestTypes.OTHER;
