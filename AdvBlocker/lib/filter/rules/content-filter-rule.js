(function (advblocker, api) {
    ...
    function Wildcard(pattern) {
        ...
        function wildcardToRegex(pattern) {
            ...
            pattern = advblocker.utils.strings.replaceAll(pattern, '\\*', '[\\s\\S]*');
            pattern = advblocker.utils.strings.replaceAll(pattern, '\\?', '.');
            return '^' + pattern + '$';
        }
        ...
        function extractShortcut(pattern) {
            ...
            var wildcardChars = ['*', '?'];
            var startIndex = 0;
            var endIndex = advblocker.utils.strings.indexOfAny(pattern, wildcardChars);
            ...
            while (endIndex >= 0) {
                ...
                endIndex = advblocker.utils.strings.indexOfAny(pattern.substring(startIndex), wildcardChars);
    ...
    var ContentFilterRule = function (ruleText, filterId) {
        ...
        while (ruleStartIndex !== -1) {
            ...
            attributeValue = advblocker.utils.strings.replaceAll(attributeValue, '""', '"');
