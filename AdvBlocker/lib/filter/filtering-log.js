 ...
 */
advblocker.filteringLog = (function (advblocker) {
    'use strict';

    const REQUESTS_SIZE_PER_TAB = 1000;

    const backgroundTabId = advblocker.BACKGROUND_TAB_ID;
    const backgroundTab = {
        tabId: backgroundTabId,
        title: advblocker.i18n.getMessage('background_tab_title'),
    };

    const tabsInfoMap = Object.create(null);
    let openedFilteringLogsPage = 0;

    const extensionURL = advblocker.getURL('');

    // Force to add background tab if it's defined
    if (advblocker.prefs.features.hasBackgroundTab) {
    ...
    function addTab(tab) {
        ...
        if (tabInfo) {
            advblocker.listeners.notifyListeners(advblocker.listeners.TAB_ADDED, tabInfo);
        }
    }
    ...
    function removeTabById(tabId) {
        ...
        if (tabInfo) {
            advblocker.listeners.notifyListeners(advblocker.listeners.TAB_CLOSE, tabInfo);
        }
    ...
    function updateTab(tab) {
        ...
        const tabInfo = updateTabInfo(tab);
        if (tabInfo) {
            advblocker.listeners.notifyListeners(advblocker.listeners.TAB_UPDATE, tabInfo);
        }
    }

    const isScriptRule = rule => rule instanceof advblocker.rules.ScriptFilterRule
        || rule instanceof advblocker.rules.ScriptletRule;
    ...
    const appendProperties = (destinationRule, sourceRule) => {
        ...
        if (sourceRule instanceof advblocker.rules.ContentFilterRule) {
            destinationRule.contentRule = true;
        } else if (sourceRule instanceof advblocker.rules.CssFilterRule) {
            destinationRule.cssRule = true;
        } else if (isScriptRule(sourceRule)) {
            destinationRule.scriptRule = true;
        } else if (sourceRule instanceof advblocker.rules.UrlFilterRule) {
    ...
    function pushFilteringEvent(tabInfo, filteringEvent) {
        ...
        advblocker.listeners.notifyListeners(advblocker.listeners.LOG_EVENT_ADDED, tabInfo, filteringEvent);
    }
    ...
    const addHttpRequestEvent = function (tab, requestUrl, frameUrl, requestType, requestRule, eventId) {
        ...
        const requestDomain = advblocker.utils.url.getDomainName(requestUrl);
        const frameDomain = advblocker.utils.url.getDomainName(frameUrl);
        ...
        const filteringEvent = {
            ...
            frameDomain,
            requestType,
            requestThirdParty: advblocker.utils.url.isThirdPartyRequest(requestUrl, frameUrl),
        };
    ...
    const addCosmeticEvent = function (tab, element, frameUrl, requestType, requestRule) {
        ...
        const frameDomain = advblocker.utils.url.getDomainName(frameUrl);
        const filteringEvent = {
            element: typeof element === 'string' ? element : advblocker.utils.strings.elementToString(element),
            frameUrl,
            frameDomain,
            requestType,
        };
        ...
        if (events) {
            for (let i = events.length - 1; i >= 0; i -= 1) {
                ...
                if (event.eventId === eventId) {
                    ...
                    advblocker.listeners.notifyListeners(advblocker.listeners.LOG_EVENT_UPDATED, tabInfo, event);
                    break;
                }
            }
        }
    ...
    const clearEventsByTabId = function (tabId) {
        ...
        if (tabInfo) {
            ...
            advblocker.listeners.notifyListeners(advblocker.listeners.TAB_RESET, tabInfo);
        }
    };
    ...
    const init = () => {
        ...
        // Bind to tab events
        advblocker.tabs.onCreated.addListener(addTab);
        advblocker.tabs.onUpdated.addListener(updateTab);
        advblocker.tabs.onRemoved.addListener((tab) => {
