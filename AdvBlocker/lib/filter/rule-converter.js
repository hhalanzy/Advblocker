...
(function (advblocker, api) {
    const stringUtils = advblocker.utils.strings;
    /**
     * advblocker scriptlet mask
     */
    // eslint-disable-next-line no-template-curly-in-string
    const advblocker_SCRIPTLET_MASK = '${domains}#%#//scriptlet(${args})';

    /**
     * advblocker scriptlet exception mask
     */
    // eslint-disable-next-line no-template-curly-in-string
    const advblocker_SCRIPTLET_EXCEPTION_MASK = '${domains}#@%#//scriptlet(${args})';
    ...
    /**
     * Convert string of UBO scriptlet rule to advblocker scritlet rule
    ...
    function convertUboScriptletRule(rule) {
        ...
        let template;
        if (mask.indexOf('@') > -1) {
            template = advblocker_SCRIPTLET_EXCEPTION_MASK;
        } else {
            template = advblocker_SCRIPTLET_MASK;
        }
    ...
    /**
     * Convert string of ABP scriptlet rule to advblocker scritlet rule
     * @param {string} rule UBO scriptlet rule
     */
    function convertAbpSnippetRule(rule) {
        ...
        const template = mask === ABP_SCRIPTLET_MASK
            ? advblocker_SCRIPTLET_MASK
            : advblocker_SCRIPTLET_EXCEPTION_MASK;
    ...
    /**
     * Convert external scriptlet rule to advblocker scriptlet syntax
