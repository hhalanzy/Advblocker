 ...
 * More info about ad filters stats: http://advblocker.com/en/filter-rules-statistics.html
 *
 * @constructor
 */
advblocker.hitStats = (function (advblocker) {
    ...
    var hitStatsHolder = {
        get hitStats() {
            return advblocker.lazyGet(hitStatsHolder, 'hitStats', getHitCountStats);
        }
    };
    ...
    function getHitCountStats() {
        var json = advblocker.localStorage.getItem(HITS_COUNT_PROP);
        ...
        try {
            ...
            advblocker.console.error("Error retrieve hit count statistic, cause {0}", ex);
        }
        return stats;
    }
    ...
    function sendStats() {
        ...
        var enabledFilters = advblocker.filters.getEnabledFilters();
        advblocker.backend.sendHitStats(JSON.stringify(hitStatsHolder.hitStats), enabledFilters);
        cleanup();
    }
    ...
    function saveHitsCountStats(stats) {
        ...
        throttleTimeoutId = setTimeout(function () {
            try {
                advblocker.localStorage.setItem(HITS_COUNT_PROP, JSON.stringify(stats));
            } catch (ex) {
                advblocker.console.error("Error save hit count statistic to storage, cause {0}", ex);
            }
            sendStats();
        }, 2000);
    }
    ...
    var addRuleHit = function (domain, ruleText, filterId, requestUrl) {
        ...
        if (requestUrl) {
            var requestDomain = advblocker.utils.url.getDomainName(requestUrl);
            // Domain hits
     ...
     */
    function cleanup() {
        advblocker.localStorage.removeItem(HITS_COUNT_PROP);
        advblocker.lazyGetClear(hitStatsHolder, 'hitStats');
    }
    ...
    advblocker.settings.onUpdated.addListener(function (setting) {
        if (setting === advblocker.settings.DISABLE_COLLECT_HITS && !advblocker.settings.collectHitsCount()) {
            cleanup();
        }
    });
